<template> 
  <div class="app-container">
    ...
    <el-dialog title="选择商品" :visible.sync="selectDialogVisible" width="50%" @close="closeModal">
      <el-input v-model="dialogData.listQuery.keyword" style="width: 250px;margin-bottom: 20px" size="small"
        placeholder="商品名称搜索">
        <el-button slot="append" icon="el-icon-search" @click="handleSelectSearch()"></el-button>
      </el-input>
      <el-table :data="dialogData.list" ref='dialogTable' @selection-change="handleDialogSelectionChange"
        :row-key="getRowKeys" border>
        <el-table-column type="selection" :selectable='selectEnable' :reserve-selection="true" width="60"
...
export default {
  ...
  methods: {
    selectEnable(row, rowIndex) {
      if (this.selectIds.includes(row.id)) return false
      else
        return true
    },
    closeModal() {
    ...
    handleSelectProduct() {
      ...
      this.selectIds = this.list.map(i => {
        return Number(i.productId)
      })
      this.getDialogList();

      // console.log('this.list', this.selectIds)
    },
    handleSelectSearch() {
    ...
    getDialogList() {
      fetchProductList(this.dialogData.listQuery).then(response => {
        ...
        if (this.selectIds.length) {
          this.dialogData.multipleSelection = response.data.list.filter(i => this.selectIds.includes(i.id));
          this.dialogData.multipleSelection.forEach(row => {
