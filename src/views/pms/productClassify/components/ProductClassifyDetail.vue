  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="productCate" :rules="rules" ref="productCateFrom" label-width="150px">
      ...
      <el-form-item label="分类图标：">
        <multi-upload v-model="productCate.icon" :maxCount="1" :maxSize="5"></multi-upload>
      </el-form-item>
      <!--
...
import {
  fetchList,
  createProductClassify,
  updateProductClassify,
  getProductClassify,
} from "@/api/productClassify";
  ...
  created() {
    if (this.isEdit) {
      getProductClassify(this.$route.query.id).then((response) => {
        ...
        if (this.productCate.icon && this.productCate.icon.length) {
          this.productCate.icon = [this.productCate.icon];
        } else this.productCate.icon = [];
      });
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm("是否提交数据", "提示", {
            ...
            let productCate = JSON.parse(JSON.stringify(this.productCate));
            if (productCate.icon) {
              productCate.icon = productCate.icon.join(",");
            }
            if (this.isEdit) {
              productCate.productAttributeIdList = this.getProductAttributeIdList();
              updateProductClassify(this.$route.query.id, productCate).then(
                (response) => {
                  this.resetForm(formName);
                  delete this.productCate.icon;
                  this.$message({
                    message: "修改成功",
                    type: "success",
                    duration: 1000,
              ...
              productCate.productAttributeIdList = this.getProductAttributeIdList();
              createProductClassify(productCate).then((response) => {
